...
import { LogoFullSmall } from "icons";
  ...
  return (
    <motion.header className={styles.wrapper} {...longFadeIn}>
      ...
      <div className={styles.logo}>
        <LogoFullSmall color="var(--color-foreground)" />
      </div>
      ...
      <div className={styles.right}>
        <button className={styles.iconLink} onClick={toggleTheme}>
          {theme === "light" ? (
            <IoMoon size={24} color="var(--color-foreground)" />
          ) : (
            <IoSunny size={24} color="var(--color-foreground)" />
          )}
        </button>
